	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		//printk("aka.jiang add : rt5660_lout_event set amp_mute_gpio_active high \n");
		break;

	case SND_SOC_DAPM_PRE_PMD:
		//printk("aka.jiang add : rt5660_lout_event set amp_mute_gpio_active low \n");
		//gpio_direction_output(rt5660->amp_mute_gpio, !rt5660->amp_mute_gpio_active);
		mdelay(50);
...
static int rt5660_parse_dt(struct rt5660_priv *rt5660, struct device *dev)
{
    ...
    //printk("aka.jiang add : rt5660_parse_dt enter \n");
	if (!node)
		return -ENODEV;
