...
#define GC2375H_LINK_FREQ_390MHZ		390000000
/* pixel rate = link frequency * 1 * lanes / BITS_PER_SAMPLE */
#define GC2375H_PIXEL_RATE		(GC2375H_LINK_FREQ_390MHZ * 2 * 1 / 10)
...
static int gc2375h_write_reg(struct i2c_client *client, u8 reg, u8 val)
{
	...
	//dev_info(&client->dev, "gc2375h write reg(0x%x val:0x%x)!\n", reg, val);
	buf[0] = reg & 0xFF;
	buf[1] = val;
...
gc2375h_find_best_fit(struct v4l2_subdev_format *fmt)
{
			  ...
			  struct v4l2_subdev_format *fmt)
{
	...
	mode = gc2375h_find_best_fit(fmt);
...
static int gc2375h_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_ANALOGUE_GAIN:
		...
		ret = gc2375h_write_reg(gc2375h->client,
					 GC2375H_PAGE_SELECT,
					 0x00);
		if ((usGain >= GC2375H_ANALOG_GAIN_1) &&
			 (usGain < GC2375H_ANALOG_GAIN_2)) {
			...
			ret |= gc2375h_write_reg(gc2375h->client,
				 ...
				 (temp << 2) & 0xfc);
		} else if ((usGain >= GC2375H_ANALOG_GAIN_2) &&
			 (usGain < GC2375H_ANALOG_GAIN_3)) {
			...
			ret |= gc2375h_write_reg(gc2375h->client,
				 GC2375H_ANALOG_GAIN_REG,
				 0x01);
			temp = 64 * usGain / GC2375H_ANALOG_GAIN_2;
			ret |= gc2375h_write_reg(gc2375h->client,
				 GC2375H_PREGAIN_H_REG,
				 temp >> 6);
			ret |= gc2375h_write_reg(gc2375h->client,
				 GC2375H_PREGAIN_L_REG,
				 (temp << 2) & 0xfc);
		} else if ((usGain >= GC2375H_ANALOG_GAIN_3) &&
			 (usGain < GC2375H_ANALOG_GAIN_4)) {
			ret |= gc2375h_write_reg(gc2375h->client,0x20,0x0c);
			ret |= gc2375h_write_reg(gc2375h->client,0x22,0x0e);
			ret |= gc2375h_write_reg(gc2375h->client,0x26,0x0e);
			...
			ret = gc2375h_write_reg(gc2375h->client,
				 GC2375H_ANALOG_GAIN_REG,
				 0x02);
			temp = 64 * usGain / GC2375H_ANALOG_GAIN_3;
			ret = gc2375h_write_reg(gc2375h->client,
				 GC2375H_PREGAIN_H_REG,
				 temp >> 6);
			ret = gc2375h_write_reg(gc2375h->client,
				 GC2375H_PREGAIN_L_REG,
				 (temp << 2) & 0xfc);
		} else if ((usGain >= GC2375H_ANALOG_GAIN_4) &&
			 (usGain < GC2375H_ANALOG_GAIN_5)) {
			ret |= gc2375h_write_reg(gc2375h->client,0x20,0x0c);
			ret |= gc2375h_write_reg(gc2375h->client,0x22,0x0e);
			ret |= gc2375h_write_reg(gc2375h->client,0x26,0x0e);
			ret |= gc2375h_write_reg(gc2375h->client,
				 GC2375H_ANALOG_GAIN_REG,
				 0x03);
			temp = 64 * usGain / GC2375H_ANALOG_GAIN_4;
			ret = gc2375h_write_reg(gc2375h->client,
				 GC2375H_PREGAIN_H_REG,
				 temp >> 6);
			ret = gc2375h_write_reg(gc2375h->client,
				 GC2375H_PREGAIN_L_REG,
				 (temp << 2) & 0xfc);
		}else if ((usGain >= GC2375H_ANALOG_GAIN_5) &&
			 (usGain < GC2375H_ANALOG_GAIN_6)) {
			ret |= gc2375h_write_reg(gc2375h->client,0x20,0x0c);
			ret |= gc2375h_write_reg(gc2375h->client,0x22,0x0e);
			ret |= gc2375h_write_reg(gc2375h->client,0x26,0x0e);
			ret |= gc2375h_write_reg(gc2375h->client,
				 GC2375H_ANALOG_GAIN_REG,
				 0x04);
			temp = 64 * usGain / GC2375H_ANALOG_GAIN_5;
			ret |= gc2375h_write_reg(gc2375h->client,
				 GC2375H_PREGAIN_H_REG,
				 temp >> 6);
			ret |= gc2375h_write_reg(gc2375h->client,
				 GC2375H_PREGAIN_L_REG,
				 (temp << 2) & 0xfc);
		}else if ((usGain >= GC2375H_ANALOG_GAIN_6) &&
			 (usGain < GC2375H_ANALOG_GAIN_7)) {
			...
			ret = gc2375h_write_reg(gc2375h->client,
				 GC2375H_ANALOG_GAIN_REG,
				 0x05);
			temp = 64 * usGain / GC2375H_ANALOG_GAIN_6;
			ret = gc2375h_write_reg(gc2375h->client,
				 GC2375H_PREGAIN_H_REG,
				 temp >> 6);
			ret |= gc2375h_write_reg(gc2375h->client,
				 GC2375H_PREGAIN_L_REG,
				 (temp << 2) & 0xfc);
		}else if ((usGain >= GC2375H_ANALOG_GAIN_7) &&
			 (usGain < GC2375H_ANALOG_GAIN_8)) {
			ret |= gc2375h_write_reg(gc2375h->client,0x20,0x0c);
			ret |= gc2375h_write_reg(gc2375h->client,0x22,0x0c);
			ret |= gc2375h_write_reg(gc2375h->client,0x26,0x0e);
			ret |= gc2375h_write_reg(gc2375h->client,
				 GC2375H_ANALOG_GAIN_REG,
				 0x06);
			temp = 64 * usGain / GC2375H_ANALOG_GAIN_7;
			ret |= gc2375h_write_reg(gc2375h->client,
				 GC2375H_PREGAIN_H_REG,
				 temp >> 6);
			ret |= gc2375h_write_reg(gc2375h->client,
				 GC2375H_PREGAIN_L_REG,
				 (temp << 2) & 0xfc);
		}else if ((usGain >= GC2375H_ANALOG_GAIN_8) &&
			 (usGain < GC2375H_ANALOG_GAIN_9)) {
			ret |= gc2375h_write_reg(gc2375h->client,0x20,0x0e);
			ret |= gc2375h_write_reg(gc2375h->client,0x22,0x0e);
			ret |= gc2375h_write_reg(gc2375h->client,0x26,0x0e);
			ret |= gc2375h_write_reg(gc2375h->client,
				 GC2375H_ANALOG_GAIN_REG,
				 0x07);
			temp = 64 * usGain / GC2375H_ANALOG_GAIN_8;
			ret |= gc2375h_write_reg(gc2375h->client,
				 GC2375H_PREGAIN_H_REG,
				 temp >> 6);
			ret |= gc2375h_write_reg(gc2375h->client,
			...
			ret |= gc2375h_write_reg(gc2375h->client,
				 GC2375H_ANALOG_GAIN_REG,
				 0x08);
			temp = 64 * usGain / GC2375H_ANALOG_GAIN_9;
			ret |= gc2375h_write_reg(gc2375h->client,
				 GC2375H_PREGAIN_H_REG,
				 temp >> 6);
			ret |= gc2375h_write_reg(gc2375h->client,
	...
	case V4L2_CID_VBLANK:
		...
		ret |= gc2375h_write_reg(gc2375h->client,
			 GC2375H_REG_VTS_H,
			 ((ctrl->val + gc2375h->cur_mode->height) >> 8) & 0x3f);
		ret |= gc2375h_write_reg(gc2375h->client, GC2375H_REG_VTS_L,
				       (ctrl->val + gc2375h->cur_mode->height) &
					   0xff);
		break;
			...
			const struct i2c_device_id *id)
{
	...
	gc2375h->pwdn_gpio = devm_gpiod_get(dev, "pwdn", GPIOD_OUT_LOW);
	if (IS_ERR(gc2375h->pwdn_gpio))
	...
	ret = media_entity_init(&sd->entity, 1, &gc2375h->pad, 0);
	if (ret < 0)
		goto err_power_off;
#endif
