...
#define GC5024_LINK_FREQ_420MHZ		420000000
/* pixel rate = link frequency * 1 * lanes / BITS_PER_SAMPLE */
#define GC5024_PIXEL_RATE		(GC5024_LINK_FREQ_420MHZ * 2 * 2 / 10)
...
static int gc5024_write_reg(struct i2c_client *client, u8 reg, u8 val)
{
	...
	//dev_info(&client->dev, "gc5024 write reg(0x%x val:0x%x)!\n", reg, val);
	buf[0] = reg & 0xFF;
	buf[1] = val;
...
gc5024_find_best_fit(struct v4l2_subdev_format *fmt)
{
				  ...
				  struct rkmodule_inf *inf)
{
	...
	strlcpy(inf->base.sensor, GC5024_NAME, sizeof(inf->base.sensor));
	strlcpy(inf->base.module, gc5024->module_name,
		sizeof(inf->base.module));
	strlcpy(inf->base.lens, gc5024->len_name, sizeof(inf->base.lens));
}
...
static int gc5024_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_ANALOGUE_GAIN:
		...
		ret = gc5024_write_reg(gc5024->client,
					 GC5024_PAGE_SELECT,
					 0x00);
		if ((usGain >= GC5024_ANALOG_GAIN_1) &&
			 (usGain < GC5024_ANALOG_GAIN_2)) {
			ret |= gc5024_write_reg(gc5024->client,
			...
			ret |= gc5024_write_reg(gc5024->client,
				 ...
				 (temp << 2) & 0xfc);
		} else if ((usGain >= GC5024_ANALOG_GAIN_2) &&
			 (usGain < GC5024_ANALOG_GAIN_3)) {
			ret |= gc5024_write_reg(gc5024->client,
				 GC5024_ANALOG_GAIN_REG,
				 0x01);
			temp = 64 * usGain / GC5024_ANALOG_GAIN_2;
			ret |= gc5024_write_reg(gc5024->client,
				 GC5024_PREGAIN_H_REG,
				 temp >> 6);
			ret |= gc5024_write_reg(gc5024->client,
				 GC5024_PREGAIN_L_REG,
				 (temp << 2) & 0xfc);
		} else if ((usGain >= GC5024_ANALOG_GAIN_3) &&
			 (usGain < GC5024_ANALOG_GAIN_4)) {
			ret |= gc5024_write_reg(gc5024->client,
				 GC5024_ANALOG_GAIN_REG,
				 0x02);
			temp = 64 * usGain / GC5024_ANALOG_GAIN_3;
			ret |= gc5024_write_reg(gc5024->client,
				 GC5024_PREGAIN_H_REG,
				 temp >> 6);
			ret |= gc5024_write_reg(gc5024->client,
				 GC5024_PREGAIN_L_REG,
				 (temp << 2) & 0xfc);
		}  else if ((usGain >= GC5024_ANALOG_GAIN_4) &&
			 (usGain < GC5024_ANALOG_GAIN_5)) {
			ret |= gc5024_write_reg(gc5024->client,
				 GC5024_ANALOG_GAIN_REG,
				 0x03);
			temp = 64 * usGain / GC5024_ANALOG_GAIN_4;
			ret |= gc5024_write_reg(gc5024->client,
				 GC5024_PREGAIN_H_REG,
				 temp >> 6);
			ret |= gc5024_write_reg(gc5024->client,
				 GC5024_PREGAIN_L_REG,
				 (temp << 2) & 0xfc);
		} else if ((usGain >= GC5024_ANALOG_GAIN_5) &&
			 (usGain < GC5024_ANALOG_GAIN_6)) {
			ret |= gc5024_write_reg(gc5024->client,
				 GC5024_ANALOG_GAIN_REG,
				 0x04);
			temp = 64 * usGain / GC5024_ANALOG_GAIN_5;
			ret |= gc5024_write_reg(gc5024->client,
				 GC5024_PREGAIN_H_REG,
				 temp >> 6);
			ret |= gc5024_write_reg(gc5024->client,
				 GC5024_PREGAIN_L_REG,
				 (temp << 2) & 0xfc);
		} else if ((usGain >= GC5024_ANALOG_GAIN_6) &&
			 (usGain < GC5024_ANALOG_GAIN_7)) {
			ret |= gc5024_write_reg(gc5024->client,
				 GC5024_ANALOG_GAIN_REG,
				 0x05);
			temp = 64 * usGain / GC5024_ANALOG_GAIN_6;
			ret |= gc5024_write_reg(gc5024->client,
				 GC5024_PREGAIN_H_REG,
				 temp >> 6);
			ret |= gc5024_write_reg(gc5024->client,
			...
			ret |= gc5024_write_reg(gc5024->client,
				 GC5024_ANALOG_GAIN_REG,
				 0x06);
			temp = 64 * usGain / GC5024_ANALOG_GAIN_7;
			ret |= gc5024_write_reg(gc5024->client,
				 GC5024_PREGAIN_H_REG,
				 temp >> 6);
			ret |= gc5024_write_reg(gc5024->client,
	...
	case V4L2_CID_VBLANK:
		...
		ret |= gc5024_write_reg(gc5024->client,
			 GC5024_REG_VTS_H,
			 ((ctrl->val + gc5024->cur_mode->height) >> 8) & 0x3f);
		ret |= gc5024_write_reg(gc5024->client, GC5024_REG_VTS_L,
				       (ctrl->val + gc5024->cur_mode->height) &
					   0xff);
		break;
			...
			const struct i2c_device_id *id)
{
	...
	gc5024->cur_mode = &supported_modes[0];
	...
	gc5024->pwdn_gpio = devm_gpiod_get(dev, "pwdn", GPIOD_OUT_LOW);
	if (IS_ERR(gc5024->pwdn_gpio))
	...
	ret = media_entity_init(&sd->entity, 1, &gc5024->pad, 0);
	if (ret < 0)
		goto err_power_off;
#endif
